...
function Calendar() {
    ...
    const eventdata = {
        '1': ['New Year', 'Holiday', 'New Year', 'Holiday', 'New Year', 'Holiday', 'New Year', 'Holiday'],
        '26': ['Republic Day'],
    }
                ...
                <div className="grid grid-cols-7 m-6  max-sm:m-0  w-fit  max-sm:w-full border-r">
                    {
                        dayOfTheWeek.map((day, index) => (
                            <div key={index} className="border-l border-b text-sm font-medium text-gray-500 w-[6.5rem] max-sm:w-full py-2 flex justify-center">
                                <h4>
                                    {day}
                                </h4>
                            </div>
                    ...
                    }
                    {
                        Array.from({ length: prefixDays }).map((_, index) => (
                            index === 0 ? <CalendarCell current={false} date={prevMonthDays - (prefixDays - 1 - index)} key={index} monthname={months[selectedDate.getMonth() - 1]} /> :
                    ...
                    }
                    {
                        Array.from({ length: numDays }).map((_, index) => (
                            index === 0 ?
                                <CalendarCell
                                    current={true}
                                    isToday={today.getDate() === (index + 1) && today.getFullYear() === selectedDate.getFullYear() && today.getMonth() === selectedDate.getMonth()}
                                    date={index}
                                    events={eventdata[`${index + 1}`]}
                                    key={index} monthname={months[selectedDate.getMonth()]}
                                    year={selectedDate.getFullYear()} monthNo={selectedDate.getMonth()}
                                /> :
                                <CalendarCell
                                    isToday={today.getDate() === (index + 1) && today.getFullYear() === selectedDate.getFullYear() && today.getMonth() === selectedDate.getMonth()}
                                    date={index} events={eventdata[`${index + 1}`]}
                                    key={index}
                    ...
                    }
                    {
                        Array.from({ length: nextMonthDays }).map((_, index) => (
                            index === 0 ? <CalendarCell current={false} date={0} key={index} monthname={months[selectedDate.getMonth() + 1]} /> :
